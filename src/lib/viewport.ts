 ...
 * ## 为什么需要
 *
 * chooseFit 只能在「一屏看全」和「贴宽滚动」之间选，两者在**宽度受限**的
 * 容器里都救不了：手机 390×844（容器约 366×700）算下来 contain 缩放 0.279、
 * 节点半径 21px、标签只有 5px；而贴宽增益是 1.00x，退 width 模式一点收益
 * 都没有。也就是说小屏没有逃生路径 —— 必须让用户自己放大。
 *
 ...
 * 三层，别混：
 *   1. 容器像素   —— 鼠标/手指事件给的坐标
 *   2. viewBox 单位 —— 设计稿坐标系（节点 x/y 就在这一层）
 *   3. 用户坐标   —— 变换前的 viewBox 单位，即节点的原始 x/y
 *
